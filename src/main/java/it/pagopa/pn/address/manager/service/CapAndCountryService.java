...
public class CapAndCountryService {
    ...
    private Mono<CountryModel> checkValidity(CountryModel countryModel) {
        ...
        if (countryModel.getStartValidity() != null && countryModel.getStartValidity().isAfter(now)) {
            return Mono.error(new Throwable(String.format("Cap is present in whitelist but start validity date is %s", countryModel.getStartValidity())));
        } else if (countryModel.getEndValidity() != null && countryModel.getEndValidity().isBefore(now)) {
            return Mono.error(new Throwable(String.format("Cap is present in whitelist but end validity date is %s", countryModel.getEndValidity())));
        }
