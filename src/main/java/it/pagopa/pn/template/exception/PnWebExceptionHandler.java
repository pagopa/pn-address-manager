...
public class PnWebExceptionHandler implements ErrorWebExceptionHandler {
    ...
    public Mono<Void> handle(@NonNull ServerWebExchange serverWebExchange, @NonNull Throwable throwable) {
        ...
        try {
            Problem problem;
            if (throwable instanceof PnAddressManagerException exception) {
    ...
    private Problem createProblem(PnAddressManagerException exception) {
        Problem problemDef = new Problem();
        problemDef.setType(String.valueOf(exception.getCause()));
        problemDef.setStatus(exception.getStatus().value());
        problemDef.setTitle(exception.getStatus().getReasonPhrase());
